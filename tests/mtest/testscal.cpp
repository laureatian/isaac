...
int status =clblasHscal(10, x, am, 0, 1, 1, &commandeQueue, 0, NULL, NULL);
std::cout<<"status"<<status<<std::endl;

err = clEnqueueReadBuffer(commandeQueue,am,CL_TRUE,0,10*sizeof(myhalf),result,0,NULL,NULL);
...
int status=3;
status =clblasSscal(10, x, am, 0, 1, 1, &commandeQueue, 0, NULL, NULL);
//status =clblasSaxpy(10, alpha, am, 0, 1, bm , 0, 1, 1, NULL, 0, NULL, NULL);
//sleep(5);
std::cout<<"status"<<status<<std::endl;
err = clEnqueueReadBuffer(commandeQueue,am,CL_TRUE,0,10*sizeof(float),result,0,NULL,NULL);
//sleep(3);
for(int i=0;i<10;i++){
  std::cout<<"result[i] "<<result[i]<<std::endl;
}
