...
int status =clblasHasum(10, cm, 0, am, 0, 1,dm, 1, &commandeQueue, 0, NULL, NULL);
std::cout<<"status"<<status<<std::endl;

err = clEnqueueReadBuffer(commandeQueue,cm,CL_TRUE,0,sizeof(myhalf)*10,result,0,NULL,NULL);
...
cl_mem dm = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(float)*12,NULL,NULL);
//clblasSaxpy(10, alpha, am, 0, 1, bm , 0, 1, 1, &commandeQueue, 0, NULL, NULL);
int status=3;
status =clblasSasum(10, cm,0, am, 0, 1, dm, 1, &commandeQueue, 0, NULL, NULL);
//status =clblasSaxpy(10, alpha, am, 0, 1, bm , 0, 1, 1, NULL, 0, NULL, NULL);
sleep(5);
std::cout<<"status"<<status<<std::endl;
err = clEnqueueReadBuffer(commandeQueue,bm,CL_TRUE,0,sizeof(myhalf),result,0,NULL,NULL);
sleep(3);
for(int i=0;i<10;i++){
  std::cout<<"result[i] "<<result[i]<<std::endl;
}
